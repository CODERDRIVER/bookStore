...
public class InterceptorUtil {
     ...
     * @param cookies   登录信息
     * @param librarianService  管理员服务
     * @return  返回是否验证成功
     * @throws Exception    出现异常
     */
    static boolean cookieAuthentication(HttpServletRequest httpServletRequest, Cookie[] cookies, LibrarianService librarianService) throws Exception {
        if (cookies == null) {
            //未登录，返回异常
            ...
            return loginAuthentication(httpServletRequest, claims, adminType,
                    librarianService.findByEmail(claims.get("email").asString()) == null);
        }
    }
